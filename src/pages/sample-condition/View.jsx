...
import SampleConditionModal from "./Modal";
import { deleteSampleCondition, getSampleConditions } from "./Queries";
...
export const SampleConditionPage = () => {
  const pageSizeData = [5, 10, 20, 50, 70, 100];
  const [sampleConditions, setSampleConditions] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [selectedSampleCondition, setSelectedSampleCondition] = useState(null); //Get data for editing / deleting
  ...
  const fetchSampleConditions = async () => {
    ...
    setError(null);
    try {
      const result = await getSampleConditions({
        search: searchQuery,
        pagination: {
          page: currentPage,
          page_size: pageSize,
      ...
      if (result.status === 200 || result.status === 8000) {
        setSampleConditions(result.data);
        ...
        setError(true);
        showToast("No Sample Condition Found", "warning", "Fetch Completed");
      }
    } catch (err) {
      setError(true);
      showToast("Unable to Fetch Sample Conditions", "warning", "Failed");
    } finally {
      setLoading(false);
    }
  };

  const handleDelete = async (sampleCondition = null) => {
    if (!sampleCondition) {
      Swal.fire("Error!", "Unable to Select this SampleCondition.", "error");
      return;
    }
    ...
    try {
      ...
      if (confirmation.isConfirmed) {
        const result = await deleteSampleCondition(sampleCondition.uid);
        if (result.status === 200 || result.status === 8000) {
          Swal.fire(
            "Process Completed!",
            "The Sample Condition has been deleted.",
            "success"
          );
          fetchSampleConditions();
        } else {
          console.error("Error deleting SampleCondition:", result);
          Swal.fire("Error Occurred!", `${result.message}`, "error");
        }
      }
    } catch (error) {
      console.error("Error deleting SampleCondition:", error);
      Swal.fire(
        "Unsuccessful",
    ...
    setSelectedSampleCondition(null); // Reset selected SampleCondition after deletion
  };
  // Fetch SampleConditions on initial load
  useEffect(() => {
  ...
  return (
    <SampleConditionContext.Provider
      value={{
        fetchSampleConditions,
    ...
    >
      <h4 className="py-3 mb-4">
        <span className="text-muted fw-light">Setting /</span> Sample Conditions
      </h4>
      <div className="card">
        <div className="d-flex justify-content-between align-items-center card-header mb-1">
          <h5 className="mb-0">All Sample Conditions</h5>
          <SampleConditionModal
            title="View  Sample Conditions"
            onClose={() => setSelectedSampleCondition(null)}
          />
        </div>
              ...
              <table className="table table-hover table-align-middle mb-0 table-bordered">
                ...
                <tbody className="table-border-bottom-0">
                  {loading ? (
                        ...
                        <div className="col-md-12 col-lg-12 col-sm-12 p-2">
                          ...
                          <center className="mt-1">
                            <h6 className="text-muted">
                              Fetching Sample Conditions
                            </h6>
                          </center>
                        </div>
                      </td>
                    </tr>
                  ) : error || sampleConditions.length === 0 ? (
                    <tr>
                      <td colSpan="100%">
                    ...
                    sampleConditions.map((rowData, index) => (
                      <tr key={rowData.uid}>
                        <td>{(currentPage - 1) * pageSize + index + 1}</td>
                        <td className="fw-medium">{rowData.name}</td>
                        <td className="fw-medium">{rowData.code}</td>
                          ...
                          <div className="dropdown">
                            ...
                            <div className="dropdown-menu">
                              <a
                                ...
                                href="#"
                                onClick={() => {
                                  setSelectedSampleCondition(rowData);
                                }}
