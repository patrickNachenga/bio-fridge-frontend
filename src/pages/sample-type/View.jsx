...
import SampleTypeModal from "./Modal";
import { deleteSampleType, getSampleTypes } from "./Queries";
import Swal from "sweetalert2";
import showToast from "../../helpers/ToastHelper";
import ReactLoading from "react-loading";
import usePagination from "../../hooks/usePagination";
import ReactPaginate from "react-paginate";
import { SampleTypeContext } from "../../utils/context";
import "animate.css";
...
export const SampleTypePage = () => {
    ...
    const [selectedSampleType, setSelectedSampleType] = useState(null); //Get data for editing / deleting
    ...
    const fetchSampleTypes = async () => {
        ...
        setError(null);
        try {
            const result = await getSampleTypes({
                search: searchQuery,
                pagination: {
                    page: currentPage,
                    page_size: pageSize,
            ...
            if (result.status === 200 || result.status === 8000) {
                ...
                setError(true);
                showToast("No Sample Type Found", "warning", "Fetch Completed");
            }
        } catch (err) {
            setError(true);
            showToast("Unable to Fetch Sample Types", "warning", "Failed");
        } finally {
            setLoading(false);
        }
    };

    const handleDelete = async (sampleType = null) => {
        if (!sampleType) {
            Swal.fire("Error!", "Unable to Select this SampleType.", "error");
            return;
        }
        ...
        try {
            ...
            if (confirmation.isConfirmed) {
                const result = await deleteSampleType(sampleType.uid);
                if (result.status === 200 || result.status === 8000) {
                    Swal.fire(
                        "Process Completed!",
                        "The Sample Type has been deleted.",
                        "success"
                    );
                    fetchSampleTypes();
                } else {
                    console.error("Error deleting SampleType:", result);
        ...
        setSelectedSampleType(null); // Reset selected SampleType after deletion
    };
    // Fetch SampleTypes on initial load
    useEffect(() => {
    ...
    return (
        <SampleTypeContext.Provider
            value={{ fetchSampleTypes, selectedSampleType, setSelectedSampleType }}
        >
            <h4 className="py-3 mb-4">
                <span className="text-muted fw-light">Setting /</span> Sample Types
            </h4>
            <div className="card">
                <div className="d-flex justify-content-between align-items-center card-header mb-1">
                    <h5 className="mb-0">All Sample Types</h5>
                    <SampleTypeModal
                        title="View  Sample Types"
                        onClose={() => setSelectedSampleType(null)}
                    />
                </div>
                                            ...
                                            <td colSpan="100%">
                                                <div className="col-md-12 col-lg-12 col-sm-12 p-2">
                                                    ...
                                                    <center className="mt-1">
                                                        <h6 className="text-muted">Fetching Sample Types</h6>
                                                    </center>
                                                </div>
                                            </td>
                                        </tr>
                                    ) : error || sampleTypes.length === 0 ? (
                                        <tr>
                                            <td colSpan="100%">
