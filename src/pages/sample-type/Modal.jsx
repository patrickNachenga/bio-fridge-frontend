...
import { createUpdateSampleType } from "./Queries";
...
const SampleTypeModal = () => {
    const { fetchSampleTypes, selectedSampleType, setSelectedSampleType } = useContext(SampleTypeContext)
    const [errors, setOtherError] = useState({});

    const initialValues = {
        name: selectedSampleType?.name || "",
        code: selectedSampleType?.code || "",
        description: selectedSampleType?.description || "",
        is_active: selectedSampleType?.is_active ?? true,
    };
    ...
    const handleSubmit = async (values, { setSubmitting, resetForm, setErrors }) => {
        try {
            // Check if the SampleType is being created or updated
            if (selectedSampleType) {
                values.uid = selectedSampleType.uid;
            }
            // Call the API to create or update the SampleType
            const result = await createUpdateSampleType(values);
    ...
    return (
        <>
            <button
                ...
                <i className="bx bx-edit-alt me-1"></i> Add Sample Type
            </button>
                ...
                <div className="modal-dialog modal-lg" role="document">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title" id="exampleModalLabel3">{selectedSampleType === null ? 'Create New' : 'View / Update'} SampleTypes </h5>
                            <button
                                type="button"
                                className="btn-close"
                                onClick={handleClose}
